...
(Frequency, array) = read('sample.wav') # Reading the sound file.
print(Frequency); 
print("Frequency of the signal is: " + str(Frequency));
...
print(np.abs(FourierTransformation[0:4000]))

plt.stem(scale[0:4000], np.abs(FourierTransformation[0:4000]), 'r')  # The size of our diagram
...
write("filtered_sample.wav", Frequency, NewSound) # Saving it to the file.
...
print("Critical Frequency of the highpass filter is: " + 1000/(Frequency/2));
...
print("Critical Frequency of the lowpass filter is: " + 380/(Frequency/2));
...
write("lowpass_sample.wav", Frequency, newFilteredSignal) # Saving it to the file.
